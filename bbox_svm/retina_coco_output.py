    ...
    for num in range(1, 12):
        for num1 in range(1, 9):
            ...
            if h_thresh:
                ...
                test_bool = "false"
            output_path = "../dataset/data/8cam_data/json_" + video_name + "/"
            ...
            images = [
                ...
                for file in natsorted(
                    glob.glob(
                        "../dataset/data/FDD_data_picture/" + video_name + "/*.jpg"
                    )
                )
            ]
            ...
            svm_model_weights = (
                "../states/SVM_model/svm_true_fall101_new_v2_c3095.model"
            )
            # 加载图片
            ...
            for idx, frame in enumerate(images):
                ...
                if len(predictions[0]["pred_masks"]) != 0:  # 防呆 is not
                    ...
                    binarized_image = predictions[0]["pred_masks"][0]
                    polygons = Mask(binarized_image).polygons()
                    # print(polygons)
                    m_x1, m_y1, m_x2, m_y2 = polygons.bbox()  # maskrcnn的bbox
                    horizontal_projection = np.sum(binarized_image, axis=0)
                ...
                np.savetxt(
                    "../dataset/data/excel/2cam/diff_csv/" + video_name + ".csv",
                    np.c_[bbox_r, box_top_center, box_bottom_right, frame_num],
                    delimiter=",",
                )
            # print(c-count)
